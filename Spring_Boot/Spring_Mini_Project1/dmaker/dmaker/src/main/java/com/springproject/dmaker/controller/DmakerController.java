...
public class DmakerController {
    ...
    /* POST - DmakerService의 객체 생성 함수 수행*/
    ...
    // PUT - (특정ID가진 사용자 정보 PUT)
    @PutMapping("/developer/{memberId}") //값을 가져옴
    // api응답받을 때, 바로 developer에 받지 말고 dto이용해서 받아줄것
    public DeveloperDetailDto editDeveloper(
            @PathVariable String memberId // {memberId}를 여기에 담아줌
            , @Valid @RequestBody EditDeveloper.Request request //모든 정보를 받아서 수정해야하므로
    ) {
        log.info("GET /developers HTTP/1.1)");
